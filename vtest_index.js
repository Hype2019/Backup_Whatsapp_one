...
async function zipAndUploadAllLogs() {
  ...
  for (const groupName of groupDirs) {
    ...
    //const zipPath = path.join(DOWNLOAD_DIR, groupName, 'mensagens.zip');
    ...
    // archive.pipe(output);
    // archive.file(logPath, { name: 'mensagens.txt' });
...
setInterval(zipAndUploadAllLogs, 12 * 60 * 60 * 1000);
zipAndUploadAllLogs();
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (isImage && isGroup) {
      try {
        ...
        const finalName = `${imageName}${randomInt(100)}_${timestamp.replace(/[:.]/g, '-')}.jpg`;
        ...
        let logLine = `[${timestamp}] ${sender}: ${finalName}\n`;
        fs.appendFileSync(logPath, logLine);

        const caption = msg.message.imageMessage?.caption;
        if (caption) {
          const captionLine = `[${timestamp}] ${sender} (legenda da imagem ${finalName}): ${caption}\n`;
          fs.appendFileSync(logPath, captionLine);
        }
...
async function uploadToOneDrive(accessToken, folderName, fileName, buffer) {
  try {
    ...
    if (e.response && e.response.status !== 409) {
      throw new Error('Erro ao criar pasta no OneDrive');
    }
  }
